...
export async function GET() {
  try {
    ...
    // Get tenant's business type and return only dynamic fields
    ...
    // Filter out static core fields and only return dynamic ones
    const dynamicFields = businessType.fields.filter(field => 
      field.enabled && !['name', 'costPrice', 'price', 'stock', 'minStock'].includes(field.name)
    )
