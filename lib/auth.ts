  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // Check factory managers from database first
        try {
          const client = await import('@/lib/mongodb').then(m => m.default)
          const mongoClient = await client
          const fashionDb = mongoClient.db('fashion_erp')
          const collections = await fashionDb.listCollections().toArray()
          const managerCollections = collections.filter(col => 
            col.name.includes('_factory_managers')
          )

          console.log('Checking manager collections:', managerCollections.map(c => c.name))

          for (const collection of managerCollections) {
            const managers = fashionDb.collection(collection.name)
            const manager = await managers.findOne({ email: credentials.email })
            
            console.log('Found manager:', manager ? 'Yes' : 'No', 'for email:', credentials.email)
            
            if (manager) {
              console.log('Manager found, checking password...')
              // Check if password matches (stored as plain text)
              if (manager.password === credentials.password) {
                console.log('Manager password match for:', credentials.email)
                return {
                  ...
                  storeName: manager.name + ' (Factory Manager)',
                  tenantType: 'manufacturer'
                }
              } else {
                console.log('Password mismatch for manager:', credentials.email)
              }
            }
          }
        } catch (managerError) {
          console.error('Manager auth error:', managerError)
        }
